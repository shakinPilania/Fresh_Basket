...
function Form({setSent}) 
{
  ...
  return (
    <div className='border  rounded-2xl border-[#1c1a161a] shadow-md w-full ' >
        <div className='p-6 '>
           <form className='space-y-4 min-w-[500px]' onSubmit={submitHandler}>
                {/* basic thing */}
                <div className='flex flex-row gap-3 w-full '>
                    <div className='flex-1'>
                        <label className='text-xs font-semibold text-stone-600 plusJakarta' htmlFor='name'>Your name </label>
                        <br/>
                        <input className='text-sm mt-2 outline-none border border-[#1c1a161a] rounded-2xl px-4 py-3 focus:border-green-400 focus:border-3 pr-17 plusJakarta' type="text" id="name" placeholder='Sahil Pilania ' required name='name' value={data.name} onChange={changeHandler} />
                    </div>
                    <div className='flex-1'>
                        <label className='text-xs font-semibold text-stone-600 plusJakarta' htmlFor='email'>Email address</label>
                        <br/>
                        <input className='text-sm  mt-2 outline-none border border-[#1c1a161a] rounded-2xl px-4 py-3 focus:border-green-400 focus:border-3  plusJakarta pr-17  '  required type="email" id="email" placeholder='sahilPilania@gmail.com' name='email' value={data.email} onChange={changeHandler}  />
                    </div>
                </div>
                {/* problem */}
                <div>
